<template>
  ...
  >
    <input
      ...
      ref="volume"
      type="range"
      class="my-2 w-full h-0 z-40 absolute appearance-none mt-[24px] rounded-full bg-opacity-100 focus:outline-none accent-white cursor-pointer"
      :class="({ rangeDotHidden: !isHover }, { rangeDot: isHover })"
    />
    <div
      class="absolute h-[4px] z-10 pointer-events-none mt-[6px] inset-y-0 left-0 w-0 bg-white"
      :style="`width: ${currentVolume}%;`"
    />
    <div
      class="absolute inset-y-0 left-0 w-full h-[4px] z-[-0] mt-[6px] bg-gray-500 rounded-full"
    />
  </div>
</template>
<script setup>
